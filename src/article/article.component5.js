...
class Article extends Component {
    ...
    componentDidMount() {
        fetch("http://localhost:8080/category/14")
            .then(res => res.json())
    ...
    render() {
        ...
        if (mov.id <= 59) {
            return <div className="Article">
                <div className="header-list">
                    <Link to={"/14"}>Artykuły</Link>
                    <Link to="/">Strona główna</Link>
                    <Link to={"/wiecej/14" + "/" + (Number(mov.id) + 1)}>Następny
                        artykuł &rarr;</Link>
                </div>
                <div className="header-container">{list}</div>
            </div>;
            ...
            return <div className="Article">
                <div className="header-list">
                    <Link to={"/wiecej/14" + "/" + (Number(mov.id) - 1)}>&#8592; Poprzedni
                        artykuł</Link>
                    <Link to={"/14"}>Artykuły</Link>
                    <Link to="/">Strona główna</Link>
                </div>
            ...
            return <div className="Article">
                <div className="header-list">
                    <Link
                        to={"/wiecej/14" + "/" + (Number(mov.id) - 1)}>&#8592; Poprzedni
                        artykuł</Link>
                    <Link to={"/14"}>Artykuły</Link>
                    <Link to="/">Strona główna</Link>
                    <Link to={"/wiecej/14" + "/" + (Number(mov.id) + 1)}>Następny
...
export const ArticleComponent5 = withRouter(Article);