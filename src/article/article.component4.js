...
class Article extends Component {
    ...
    componentDidMount() {
        fetch("http://localhost:8080/category/12")
            .then(res => res.json())
    ...
    render() {
        ...
        if (mov.id <= 47) {
            return <div className="Article">
                <div className="header-list">
                    <Link to={"/12" }>Artykuły</Link>
                    <Link to="/">Strona główna</Link>
                    <Link to={"/wiecej/12"+ "/" + (Number(mov.id) + 1)}>Następny
                        artykuł &rarr;</Link>
                </div>
                <div className="header-container">{list}</div>
            </div>;
            ...
            return <div className="Article">
                <div className="header-list">
                    <Link to={"/wiecej/12"+ "/" + (Number(mov.id) - 1)}>&#8592; Poprzedni
                        artykuł</Link>
                    <Link to={"/12"}>Artykuły</Link>
                    <Link to="/">Strona główna</Link>
                </div>
            ...
            <div className="header-list">
                <Link
                    to={"/wiecej/12"+ "/" + (Number(mov.id) - 1)}>&#8592; Poprzedni
                    artykuł |</Link>
                <Link to={"/12"}>Artykuły</Link>
                <Link to="/">Strona główna</Link>
                <Link to={"/wiecej/12" + "/" + (Number(mov.id) + 1)}>Następny
...
export const ArticleComponent4 = withRouter(Article);